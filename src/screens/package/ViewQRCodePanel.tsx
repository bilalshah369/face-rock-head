...
">
      ...
      <QRInfoModal
        ...
        qrImageUrl={selectedQR?.qr_image_url}
        title={`${selectedQR?.qr_type} Package QR`}
        info={[
          {label: 'Tracking ID', value: selectedQR?.tracking_id},
          {label: 'Package Type', value: selectedQR?.qr_type},
          {label: 'Status', value: selectedQR?.status},
          {label: 'Generated By', value: selectedQR?.created_by},
        ]}
        description={selectedQR?.encrypted_payload}
      />
    </div>
  );
}
